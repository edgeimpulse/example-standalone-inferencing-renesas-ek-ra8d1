...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x1d4c0,
  .duty_cycle_counts = 0xea60, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = periodic_timer_msgq_cb,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.00001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x4b0,
  .duty_cycle_counts = 0x258, .source_div = (timer_source_div_t) 0, .channel = 1, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
